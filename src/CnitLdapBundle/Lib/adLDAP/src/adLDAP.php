<?php
use Saba\PullBundle\Biz\ApiConfigBiz;
use Saba\MongoBundle\Lib\MongoDBManager;
...
class adLDAP {
    ...
    * @var string
    */   
	protected $accountSuffix = "@saba.com";
    ...
    * @var string
    */
	protected $baseDn = "DC=saba,DC=com";
	
	/**
    * The top base dn for your domain
    * 
    * If this is set to null then adLDAP will attempt to obtain this automatically from the rootDSE
    * 
    * @var string
    */
	protected $topBaseDn = "DC=saba,DC=com"; 
    ...
    * @var array
    */
    protected $domainControllers = array("hqdc1.saba.com");
    ...
    * @var string
    */
	protected $adminUsername = NULL;//'crmsmtp';
    protected $adminPassword = NULL;//'!Dymamics2013@Saba';
    ...
    * @return string
    */
    public function setTopBaseDn($topBaseDn) {
        return $this->topBaseDn = $topBaseDn;
    }
    ...
    * @return string
    */
    public function getTopBaseDn() {
        return $this->topBaseDn;
    }
    ...
    */
	...
	public function getConfigApi(){
		...
		$mongoDBManager = MongoDBManager::getDBManager();	
		
		$result = $mongoDBManager -> findOne(ApiConfigBiz::getBizEntityName(), 'Key', "LDAPApi");
    ...
    */
    public function connect() 
    {
		...
		$server = $port = $user = $password = NULL;
		if (!is_null($config))
		{
			$hosts = explode(":", $config["Value"]["Url"]);
			$server = $hosts[0]; //"172.16.4.106";
			$port = $hosts[1]; //"5432";
			$user = $config["Value"]["Account"]; //"admin";
			$password = $config["Value"]["Password"];
			$token = json_decode($config["Value"]["Token"]);
			
			if (!isset($token->accountSuffix) || !isset($token->baseDN) || !isset($token->topDN))
				throw new \Exception("mess.ldap.accountsufandbasedn.invalid");
			
			$this->setAdminUsername($user);
			$this->setAdminPassword($password);
			$this->setPort($port);
			$this->setAccountSuffix($token->accountSuffix);
			$this->setBaseDN($token->baseDN);
			$this->setTopBaseDN($token->topDN);
			$this->setDomainControllers(array($server));
			if($this->adPort == self::ADLDAP_LDAPS_PORT)
			{
				$this->setUseSSL(true);
			}
		}
		...
		{
			throw new \Exception("mess.ldap.configuration.required");			
		}
		...
		//ldap_set_option($this->ldapConnection, LDAP_OPT_SIZELIMIT, 100); //PhongLam add this line
