...
class adLDAPGroups {
    ...
    * 
    * @param string $group The group to query: this is group name<samaccountname>
    * @param bool $recursive Recursively get group members<true for getting group members>
    * @return array
    */
    ...
    */
    public function search($sAMAaccountType = adLDAP::ADLDAP_SECURITY_GLOBAL_GROUP, $includeDescription = false, $search = "*", $sorted = true) {
        ...
        $fields = array("member","memberof","cn","description","distinguishedname","objectcategory","samaccountname", "objectguid");
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields);
        $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
        ...
        for ($i=0; $i<$entries["count"]; $i++){
            if ($includeDescription && strlen($entries[$i]["description"][0]) > 0 ) {
				...
				{
					if($entries[$i]["member"]["count"] > 0)
					{
						for ($j=0; $j < $entries[$i]["member"]["count"]; $j++) { 
							$members[] = $entries[$i]["member"][$j];
						}
					}
				}
				if(isset($entries[$i]["memberof"]))
				{
					if($entries[$i]["memberof"]["count"] > 0)
					{
						for ($k=0; $k < $entries[$i]["memberof"]["count"]; $k++) { 
							$memberof[] = $entries[$i]["memberof"][$k];
						}
					}
				}
            	$newArray = array($entries[$i]["cn"][0], 
            						$entries[$i]["distinguishedname"][0],
									$members, $memberof);
                array_push($groupsArray, $newArray);
