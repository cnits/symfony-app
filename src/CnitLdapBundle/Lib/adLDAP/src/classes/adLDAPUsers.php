...
class adLDAPUsers {
    ...
    */
    public function create($attributes)
    {
        ...
        // Add the entry
        $result = @ldap_add($this->adldap->getLdapConnection(), "CN=" . $add["cn"][0] . ", " . $container . "," . $this->adldap->getTopBaseDn(), $add);
        if ($result != true) { 
            return false; 
        }
    ...
    public function info($username, $fields = NULL, $isGUID = false)
    {
        ...
        if ($fields === NULL) { 
            $fields = array("samaccountname","userprincipalname","displayname",
            				...
            				"useraccountcontrol","dlmemsubmitperms","dlmemrejectperms", "enabled", 'directreports', 'cn'); 
        }
        if (!in_array("objectsid", $fields)) {
            $fields[] = "objectsid"; 
        }
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getTopBaseDn(), $filter, $fields);
            ...
            if ($entries[0]['count'] >= 1) {
                if (in_array("memberof", $fields)) {
                    ...
                    if ($this->adldap->getRealPrimaryGroup() && isset($entries[0]["primarygroupid"][0]) && isset($entries[0]["objectsid"][0])){
                        ...
                        $entries[0]["memberof"][] = "CN=Domain Users,CN=Users," . $this->adldap->getTopBaseDn();
                    }
    ...
    */
    public function passwordExpiry($username, $isGUID = false) 
    {
        ...
        if ($pwdLastSet === '0') {
         ...
         $sr = ldap_read($this->adldap->getLdapConnection(), $this->adldap->getTopBaseDn(), 'objectclass=*', array('maxPwdAge'));
         if (!$sr) {
             return false;
         }
    ...
    */
    public function all($includeDescription = false, $search = "*", $sorted = true)
    {
		...
		$fields = array("samaccountname","userprincipalname","displayname",
            				...
            				"scriptPath","profilepath","primarygroupid","objectsid",
            				"pager","wwwhomepage","facsimiletelephonenumber",
            				"useraccountcontrol","dlmemsubmitperms","dlmemrejectperms", "enabled", 'cn'); 
		...
		do {
	         ...
	         $result  = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getTopBaseDn(), $filter, $fields);
	         $entries = ldap_get_entries($this->adldap->getLdapConnection(), $result);
	         foreach ($entries as $e) {
				 array_push($usersArray, $e);
	         }
	         ldap_control_paged_result_response($this->adldap->getLdapConnection(), $result, $cookie);
    ...
    */
    public function move($username, $container) 
    {
        ...
        $newBaseDn = strtolower($newContainer) . "," . $this->adldap->getTopBaseDn();
